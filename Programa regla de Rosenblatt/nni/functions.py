...
def ackley_gradient_x(x, y):
    return 2 * x * math.exp(-0.2 * math.sqrt(0.5 * (x**2 + y**2))) / math.sqrt(
        0.5 * (x**2 + y**2)
    ) + 2 * math.pi * math.sin(2 * math.pi * x) * math.exp(
...
def ackley_gradient_y(x, y):
    return 2 * y * math.exp(-0.2 * math.sqrt(0.5 * (x**2 + y**2))) / math.sqrt(
        0.5 * (x**2 + y**2)
    ) + 2 * math.pi * math.sin(2 * math.pi * y) * math.exp(
        0.5 * (math.cos(2 * math.pi * x) + math.cos(2 * math.pi * y))
    )
...
class BealeProblem(optimizationProblem2D):
    def __init__(self):
        super().__init__(
            ...
            beale_gradient_y,
            initial_params=[0, 0],
            search_space=[-4.5, 4.5, -4.5, 4.5],
            optimal_params=[3, 0.5],
        )
...
class GradientDescentOptimizer(Optimizer):
    ...
    def next_params(self, gradient, params):
        return [
            param - self.learning_rate * grad for param, grad in zip(params, gradient)
        ]
    ...
    def _converged(self, params, updated_params):
        return all(
            abs(param - updated_param) < self.tolerance
...
def datosSinteticos():
    ...
    X, y = pd.DataFrame(
        np.vstack(
            [
                np.random.multivariate_normal(mean1, cov1, size=int(n_samples / 2)),
                np.random.multivariate_normal(mean2, cov2, size=int(n_samples / 2)),
            ]
        ),
        columns=["x1", "x2"],
    ), pd.Series([0] * int(n_samples / 2) + [1] * int(n_samples / 2), name="target")
    return X, y
